<template>
  ...
  >
    ...
    >
      ...
      <!-- 表头右侧两个模块 -->
      <div style="display: flex; justify-content: space-around; width: 13vw">
        <div>
          <img
            src="../assets/e.png"
            style="height: 2.5vh; margin-top: 1vh; margin-left: 2vw"
          />
        </div>
        <div>
          <div>{{ chn }}</div>
        ...
        <div>
          <img
            :src="photoURL"
            style="width: 2.5vw; height: 5vh; border-radius: 25px"
          />
        </div>
      </div>
      <!-- 表头右侧两个模块 -->
...
export default {
  ...
  methods: {
    ...
    // 获取用户名和相片
    select(){
         this.$axios('http://so.newsteel.ai:8888/shiro/web/authc/isPermitted?permission=prod:sop:view&token='+token).then((res)=>{
                    
        })
    }
  },
